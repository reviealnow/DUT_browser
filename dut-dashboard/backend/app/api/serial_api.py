...
def bundle_session_context(session_dir: Path, log_path: Path) -> list[Path]:
    """Copy the context captured *while this log was recording* into
    ``session_dir/context/<kind>/`` so the ZIP carries the on-arrival picture.

    Selection is by the log's own time window. Until P73 this copied the newest
    survey of every DUT regardless of the log's age, so a ZIP for last week's log
    was handed today's survey; an empty window now yields nothing at all.

    Best-effort: bundle_context swallows its own errors, so a missing or
    unreadable capture can never fail the download.
    """
